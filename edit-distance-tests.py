...
class TestEditDistance(unittest.TestCase):	
		...
		def test_random_words(self):
			test_cases = zip(['islander','mart','kitten','intention'], ['slander', 'karma','sitting','execution'])
			test_results = [1,3,3,5]
			for i in range(len(test_cases)):
				self.assertEqual(edit_distance(test_cases[i][0], test_cases[i][1]), test_results[i])
